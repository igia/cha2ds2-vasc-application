...
describe('login', () => {
    ...
    it('should fail to login with bad password', async () => {
        navbarPage.signInMenu.click();
        await browser.sleep(5000);
        browser.wait(EC.urlContains('auth/realms'), 20000, 'Login url not present').catch(() => {});
        expect(await browser.getCurrentUrl()).to.containIgnoreCase('auth/realms');
        await signInPage.login('admin', 'foo');
        browser.wait(signInPage.loginError.isPresent(), 6000, 'Login error not present after wait of 3 seconds.');
    ...
    it('should login successfully with admin account', async () => {
        navbarPage.signInMenu.click();
        await browser.sleep(5000);
        browser.wait(EC.urlContains('auth/realms'), 15000, 'Login url not present').catch(() => {});
        expect(await browser.getCurrentUrl()).to.containIgnoreCase('auth/realms');

        await signInPage.login('admin', 'admin');
        browser.wait(EC.urlContains('/'), 5000, 'Page is not redirected to the data-pipeline list page after wait of 5 seconds.');
        browser.waitForAngularEnabled(true);
